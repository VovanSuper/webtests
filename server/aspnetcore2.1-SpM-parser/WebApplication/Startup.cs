...
namespace WebApplication
{
   public class Startup
   {
      ...
      public void Configure(IApplicationBuilder app, IHostingEnvironment env) => app
            ...
            });//app.Run(async (context) =>//   {//      var spGrabber = app.ApplicationServices.GetRequiredService<ISpGrabber>();//      var prices = spGrabber.GetSportMasterPrices();//      await context.Response.WriteAsync(@"Prices are: \n ");//      foreach (var item in prices)//      {//         await context.Response.WriteAsync($"\n{item}");//      }//   });
   }
}
